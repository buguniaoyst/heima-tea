...
<div class="layui-form layui-form-pane"  style="margin-top: 15px;margin-left: 5px;">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <input class="layui-input" name="assistant" id="assistant">
        </div>
        <label class="layui-form-label">讲师</label>
        <div class="layui-input-inline">
            <input class="layui-input" name="teacherName" id="teacherName">
        </div>
        ...
        <div class="layui-inline">
            <button class="layui-btn layui-btn-radius" id="schedulingQueryBtn">查询</button>
            <button class="layui-btn layui-btn-radius" id="diaryQueryExport">导出数据</button>
            <button class="layui-btn layui-btn-radius" id="generateAssistantSchedule">生成助教排班信息</button>
        </div>
    </div>
...
<script>
    ...
    layui.use(['laypage', 'layer','laydate','jquery','form'],function() {
        ...
        $(function(){
            ...
            $.ajax({
                type: "GET",
                url: "${pageContext.request.contextPath}/rest/course_schedule_detail/getCourseScheduleDetailList",
                //记得加双引号  T_T
        ...
        //拼接条件查询排班信息
        $("#schedulingQueryBtn").click(function () {
            //班级名称
            ...
            $.ajax({
                type: "POST",
                url: "${pageContext.request.contextPath}/rest/course_schedule_detail/getScheduleByExample",
                data: {className:className,assistant:assistant,teacherName:teacherName,fromDate:fromDate,endDate:endDate},
                dataType: "json",
                success: function(data){
                    layui.use(['laypage', 'layer','laydate','jquery','form'],function() {
                        var laydate = layui.laydate;
        ...
        $('#diaryQueryExport').click(function () {
            //助教
            var assistant = $("#assistant").val()?$("#assistant").val():"";
            //讲师
            var teacherName = $("#teacherName").val()?$("#teacherName").val():"";
            //fromDate
            var fromDate = $("#fromDate").val()?$("#fromDate").val():"";
            //endDate
            var endDate = $("#endDate").val()?$("#endDate").val():"";
            var form =  $("<form>").attr({
                "action":"/rest/course_schedule_detail/exportCourseScheduleList?assistant="+assistant+"&teacherName="+teacherName+"&fromDate="+fromDate+"&endDate="+endDate,
                "method":"POST"
            });
         ...
         * 生成助教排班信息
         */
        $('#generateAssistantSchedule').click(function () {
            //助教
            var assistant = $("#assistant").val()?$("#assistant").val():"";
            //讲师
            var teacherName = $("#teacherName").val()?$("#teacherName").val():"";
            ...
            var endDate = $("#endDate").val()?$("#endDate").val():"";
            var form =  $("<form>").attr({
                "action":"/rest/course_schedule_detail/generateAssistantSchedule?assistant="+assistant+"&teacherName="+teacherName+"&fromDate="+fromDate+"&endDate="+endDate,
                "method":"POST"
            });
            $(document.body).append(form);
            form.submit();
